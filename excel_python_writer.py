...
# 엑셀 파일 생성
workbook = xlsxwriter.Workbook("c:/test/examplee_details.xlsx")
...
for row_num, employee in enumerate(employee_data):
    ...
    worksheet.write(row_num + 1, 4, employee[4], money_format)

# 조건부 서식 추가 (연봉 5000만원 이상인 경우 배경색 편집)
...
worksheet.write(total_row, 4, f'=SUM(E2:E{total_row})', money_format)

# 요약 시트에 데이터 추가
summary_sheet.write(0, 0, "부서별 인원 및 평균 연봉", workbook.add_format({'bold': True, 'font_size': 14}))
summary_sheet.write(1, 0, "부서", header_format)
summary_sheet.write(1, 1, "인원수", header_format)
summary_sheet.write(1, 2, "평균 연봉", header_format)

# 부서 목록(중복제거)
departments = list(set([employee[2] for employee in employee_data]))

# 부서별 통계 계산 및 기록
for i, dept in enumerate(departments):
    row = i + 3
    dept_employees = [e for e in employee_data if e[2] == dept]
    count = len(dept_employees)
    average_salary = sum([e[4] for e in dept_employees]) / count

    summary_sheet.write(row, 0, dept, header_format)
    summary_sheet.write(row, 1, count, header_format)
    summary_sheet.write(row, 2, average_salary, money_format)
